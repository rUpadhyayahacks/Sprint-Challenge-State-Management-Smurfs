...
Actions is an event that is triggered by the user. The reducer holds the initial state. The store holds the state and data and takes information from the reducers.

The store is read only and requires a dispatch action thats why its known as single source of truth


- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is immutable. Component state is only accessible from the component itself unless its passed through with props. 

Its better to use application state with larger projects.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-thunk allows us to write action creators that return a function instead of an action. Thunk can be used to delay the dispatch of an action, or to dispatch only if a certain condition is met. 


- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

I guess redux because i used it on this sprint. It allows you to break down the components easier while being flexible
